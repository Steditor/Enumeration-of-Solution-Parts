...
const fn unweighted_algorithms<G, I, ED>() -> [AlgorithmType<G, I>; 5]
where
    G: Graph<I, ED>,
    I: Index,
    ED: EdgeData,
...
pub fn unweighted_experiment_set() -> ExperimentSet {
    ExperimentSet {
        ...
        aggregate: |options| {
            ...
            aggregate_apsd_on_sssd_worst_case(
                options,
                &unweighted_algorithms::<UndirectedAdjacencyArrayGraph<u32, u8>, _, _>(),
            );
        },
        ...
        .filter(|&size| max_size.is_none_or(|max| size <= max))
    {
        ...
        for (edge_probability, parameter_label) in edge_generation_parameters {
            ...
            for i in 1..=instances_per_size {
                ...
                runner::run_cachable_experiment::<
                    _,
                    _,
                    _,
                    _,
                    ...
                    (),
                    _,
                >(&mut generator, options, runs_per_instance, algorithms)
                .unwrap();
            }
        }
    }
...
fn run_apsd_on_sssd_worst_case<D>(
    options: &mut ExperimentOptions,
        ...
        .filter(|(_, n)| max_size.is_none_or(|max| *n <= max))
    {
        ...
        if options.run_algorithms {
            log::info!("Run APSD algorithm for SSSD worst-case instance with clique size {k}.");
...
fn aggregate_apsd_on_sssd_worst_case<G, I, ED, D>(
