...
where
    ...
    ED: EdgeData,
{
    ExperimentAlgorithm::EnumerationAlgorithm("enum-bfs", |(graph, source)| {
    ...
    ED: EdgeData,
{
    pub fn new(graph: &'a G, source: I) -> Self {
        ...
        distances[source.index()] = Some(I::zero());
        ...
        } = self
        {
            if let Some(u) = queue.pop_front() {
                let d = unsafe {
                    ...
                    distances[u.index()].unwrap_unchecked()
                };
                let new_d = d + I::one();
...
pub const fn algorithm_bfs<G, I, ED>() -> AlgorithmType<G, I, I>
where
    G: Graph<I, ED>,
    I: Index,
    ED: EdgeData,
{
    ExperimentAlgorithm::TotalTimeAlgorithm("bfs", |(graph, source)| Ok(sssd(graph, *source)))
}

/// Compute unweighted hop SSSD via BFS
pub fn sssd<G, I, ED>(graph: &G, source: I) -> Vec<Option<I>>
where
    G: Graph<I, ED> + ?Sized,
    I: Index,
    ED: EdgeData,
{
    let mut distances = vec![None; graph.num_vertices().index()];
    let mut queue = VecDeque::with_capacity(graph.num_vertices().index());
...
mod test {
    ...
    #[test]
    fn test_undirected_sssd_crls_22_3() {
        let graph = undirected_crls_22_3();
        let distances = sssd(&graph, 1);
        assert_eq!(distances, [1, 0, 2, 3, 2, 1, 2, 3].map(Some));
    }

    #[test]
    fn test_directed_sssd_crls_22_3() {
